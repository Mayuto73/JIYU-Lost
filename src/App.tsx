...
export default function App() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
          <div className="bg-[#8B3A3A] p-8 text-center">
            <div className="w-20 h-20 mx-auto bg-white/20 rounded-full flex items-center justify-center mb-4">
              <School className="w-12 h-12 text-white" />
            </div>
            <h1 className="text-2xl font-serif text-white tracking-wider">自由学園</h1>
            <p className="text-white/80 text-sm mt-2 tracking-widest">落とし物管理システム</p>
          </div>
          <div className="p-8 space-y-6">
            <div className="text-center space-y-2">
              <h2 className="text-lg font-medium text-gray-800">教職員・生徒用ログイン</h2>
            ...
            <form onSubmit={handleEmailAuth} className="space-y-4">
              ...
              <Button 
                type="submit" 
                className="w-full h-12 text-lg bg-[#8B3A3A] hover:bg-[#712F2F] text-white"
              >
                {isSignUp ? '新規登録' : 'ログイン'}
              </Button>
            </form>
            ...
            <div className="text-center space-y-2 pt-2">
              ...
              <Button variant="ghost" onClick={() => setShowLogin(false)} className="text-gray-500">
                ログインせずに閲覧する
              </Button>
            </div>
          </div>
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {filteredItems.map(item => (
          <Card 
            key={item.id} 
            className={`cursor-pointer transition-all hover:shadow-md border border-gray-200 overflow-hidden ${item.status === '受取済' ? 'opacity-60 grayscale' : ''}`}
            onClick={() => {
            ...
            <div className="aspect-square w-full bg-gray-100 relative overflow-hidden">
              ...
              {item.status === '受取済' && (
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                  <span className="bg-[#8B3A3A] text-white px-4 py-2 rounded-full font-bold flex items-center gap-2 shadow-lg">
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      {/* Header - 自由学園スタイル */}
      <header className="bg-[#8B3A3A] text-white border-b border-[#712F2F] sticky top-0 z-10 shadow-md">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
              <School className="w-6 h-6 text-white" />
            </div>
            <h1 className="text-xl font-bold tracking-wide">自由学園 落とし物掲示板</h1>
          </div>
          <div className="flex items-center gap-4">
            {user ? (
              <>
                <span className="text-sm text-white/80 hidden sm:inline-block">{user.displayName}</span>
                <Button variant="ghost" size="icon" onClick={handleLogout} title="ログアウト" className="text-white hover:bg-white/20">
                  <LogOut className="h-5 w-5" />
                </Button>
              </>
            ) : (
              <Button variant="outline" size="sm" onClick={() => setShowLogin(true)} className="bg-white/10 border-white/30 text-white hover:bg-white/20 hover:text-white">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        <Tabs defaultValue="高等部" className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-6 bg-gray-100 p-1 rounded-full">
            <TabsTrigger 
              value="高等部" 
              className="data-[state=active]:bg-[#8B3A3A] data-[state=active]:text-white rounded-full transition-all"
            >
              高等部
            </TabsTrigger>
            <TabsTrigger 
      ...
      {user && (
        <Dialog open={isPosting} onOpenChange={setIsPosting}>
          <DialogTrigger asChild>
            <Button 
              className="fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-lg bg-[#8B3A3A] hover:bg-[#712F2F] text-white" 
              size="icon"
            >
          ...
          <DialogHeader>
            <DialogTitle className="text-[#8B3A3A]">落とし物を投稿</DialogTitle>
            <DialogDescription>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            <div className="space-y-2">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                {imagePreview ? (
                  ...
                  <div className="py-6 flex flex-col items-center text-gray-500 space-y-4">
            ...
            <DialogFooter className="pt-4">
              ...
              <Button type="submit" disabled={!title || isSubmitting} className="bg-[#8B3A3A] hover:bg-[#712F2F] text-white">
            ...
            <>
              <div className="relative w-full bg-gray-100">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  <span className="bg-white/80 backdrop-blur-sm text-[#8B3A3A] px-3 py-1 rounded-full text-xs font-medium border border-[#8B3A3A]/20">
                    {selectedItem.category}
                  </span>
                  {selectedItem.status === '受取済' && (
                    <span className="bg-[#8B3A3A] text-white px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1">
              ...
              <div className="p-6 space-y-4">
                ...
                {selectedItem.description && (
                  <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <p className="whitespace-pre-wrap text-sm text-gray-700">{selectedItem.description}</p>
                  </div>
                )}
                ...
                <div className="pt-4 flex flex-col gap-3">
                  ...
                  {user && (user.uid === selectedItem.authorUid || isAdmin || user.uid === selectedItem.claimedByUid) && selectedItem.status === '未受取' && (
                    <Button 
                      className="w-full bg-[#8B3A3A] hover:bg-[#712F2F] text-white" 
                      size="lg"
